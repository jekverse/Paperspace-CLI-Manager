...
def start_notebook(notebook_id):
    new_notebook_id = modules.utils.start_notebook(notebook_id)
    ...
    if new_notebook_id is False:
        ...
        if option == '1':
            notebook_id = modules.utils.retry_until_success()
        elif option == '2':
            new_machine_type = modules.utils.list_available_gpus(from_main_menu=True)
            if new_machine_type:
                current_machine_type = new_machine_type
                notebook_id = modules.utils.retry_until_success(current_machine_type)
        else:
